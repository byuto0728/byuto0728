	...
	<?php
		...
		if(isset($_POST['submit2'])){
			if(isset($delete) && isset($pass2)){
				...
				foreach ($results as $text){
					if($text['id'] == $delete && $text['password'] == $pass2){
						$id = $delete;
					...
					}elseif($text['id'] == $delete && $text['password'] != $pass2){
		...
		if(isset($_POST['submit3'])){
			if(isset($edit) && isset($pass3)){
				...
				$results = $stmt->fetchAll(); //複数のデータを配列resultsに返す
				foreach ($results as $text){
					if($text['id'] == $edit && $text['password'] == $pass3){
						$edit_name=$text['name'];
						$edit_comment=$text['comment'];
						$edit_nu=$edit;
						$Reception="投稿番号".$edit."を編集します"."<br>";
					}elseif($text['id'] == $edit && $text['password'] != $pass3){
